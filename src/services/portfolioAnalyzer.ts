import { PortfolioAction, PortfolioSecurity, PortfolioSummary, SecurityInfo, FundAnalysis, RebalanceResult, RebalanceRecommendation, CustomSecurity } from '../types';
import { Decimal } from 'decimal.js';
import { APP_CONFIG } from '../config/constants';

/**
 * Portfolio analysis service that processes portfolio actions and calculates comprehensive metrics.
 * 
 * This service handles:
 * - Processing buy/sell/deposit transactions
 * - Calculating current portfolio value using real-time market data
 * - Computing profit/loss with tax considerations
 * - Analyzing fund type distribution (bonds vs shares)
 * - Managing custom securities (pension funds, savings accounts, etc.)
 * 
 * @example
 * ```typescript
 * const analyzer = new PortfolioAnalyzer(targetDistribution, customSecurities);
 * const summary = analyzer.analyzePortfolio(actions, securitiesInfo);
 * ```
 */
export class PortfolioAnalyzer {
  ...
  /**
   * Analyzes a complete portfolio based on transaction history and current market data.
   * 
   * This method processes all portfolio actions, fetches current market prices,
   * and calculates comprehensive portfolio metrics including:
   * - Total market value and net buy value
   * - Profit/loss calculations
   * - Fund type distribution analysis
   * - Available cash calculation
   * - Custom securities integration
   * 
   * @param actions - Array of portfolio transactions (buy, sell, deposit)
   * @param securitiesInfo - Current market data for securities
   * @returns Comprehensive portfolio summary with all calculated metrics
   */
  analyzePortfolio(actions: PortfolioAction[], securitiesInfo: Record<string, SecurityInfo>): PortfolioSummary {
    ...
    const regularSecuritiesNetBuyValue = totalNetBuyValue;
    ...
    const remainingCash = totalDeposits.minus(regularSecuritiesNetBuyValue).minus(totalCommissions);
